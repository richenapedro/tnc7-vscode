...
const MANUAL_M_DOC = {
    ...
    1: {
        ...
        purpose: "Optionally stops program run, optionally stops the spindle, and optionally switches the coolant supply off. Behavior depends on the machine manufacturer.",
        source: "HEIDENHAIN TNC7 basic User's Manual (10/2023)"
    },
    2: {
        title: "M2",
        function: "Stop, return to program start (machine-dependent reset)",
        purpose: "Stops program execution and the spindle, switches the coolant supply off, returns to the beginning of the program, and can optionally reset program information. Behavior depends on machine parameter resetAt (No. 100901).",
        source: "HEIDENHAIN TNC7 basic User's Manual (10/2023)"
    },
    3: {
        title: "M3",
...
const ALL_M_DOC = { ...MANUAL_M_DOC, ...M_DOC };
function renumberText(fullText) {
...
function buildMHover(code) {
    ...
    if (doc.function)
        md.appendMarkdown(`- **Function:** ${doc.function}\n`);
    if (doc.purpose)
        md.appendMarkdown(`- **Description:** ${doc.purpose}\n`);
    if (doc.source)
        md.appendMarkdown(`- **Source:** \`${doc.source}\`\n`);
...
function activate(context) {
    const renumberCmd = vscode.commands.registerCommand("tnc7.renumber", async () => {
        ...
        if (!editor)
            return;
        if (editor.document.languageId !== "tnc7h")
            return;
